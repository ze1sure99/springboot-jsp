...
if (tMSG != null && !tMSG.equals("")) {		  
	...
	if (tResult.getValueNoNull("MessageType").equals(IPaymentMerchant.MESSAGETYPE_QREQ)) {
		System.out.println("查询信息处理中...3");
		  ...
		  tPayerValueList.add("222");//根据定义的账单类型，设定账单查询条件信息1
		  tPayerValueList.add("12");//根据定义的账单类型，设定账单查询条件信息2		
		  ...
		  tVerifyDomainList.add("234");//根据定义的账单类型，设定账单验证信息1
		  tVerifyDomainList.add("二班");//根据定义的账单类型，设定账单验证信息2
		  ...
		  //指定取第几个商户号
		  tBase64Message = tPaymentMerchant.getBase64Merchant("true",1);//true表示有符合条件的账单，false表示没有符合条件的账单
